...
void OLED_Init(void) {
	...
	OLED_WriteCommand(0x1F);  // 0x0E~0x3F, 0x1F=31 => 32行 (128x32)
	...
	OLED_WriteCommand(0x02);  // 0x02 for 128x32
...
void OLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height) {
	...
	for (j = Page; j < Page1; j++) {
		if (X >= 0 && X <= 127 && j >= 0 && j <= 3)	 // 超出屏幕的内容不显示
		{
			/*设置光标位置为相关页的指定列*/
			OLED_SetCursor(j, X);
...
void OLED_ClearArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height) {
	...
	for (j = Y; j < Y + Height; j++)  // 遍历指定页
	{
		...
		{
			if (i >= 0 && i <= 127 && j >= 0 && j <= 31)  // 超出屏幕的内容不显示
			{
	...
	for (j = 0; j < (Height - 1) / 8 + 1; j++) {
		...
		for (i = 0; i < Width; i++) {
			if (X + i >= 0 && X + i <= 127)	 // 超出屏幕的内容不显示
			{
				...
				if (Page + j >= 0 && Page + j <= 3)	 // 超出屏幕的内容不显示
				{
					/*显示图像在当前页的内容*/
