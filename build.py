...
'''
    ...
    def _is_valid_compiler_path(self, compiler_path):
        """检查编译器路径是否有效"""
        if not compiler_path:
            return False

        # 检查路径是否包含路径分隔符（表示是完整路径而不是命令名）
        if '/' not in compiler_path and '\\' not in compiler_path:
            return False

        # 转换为Path对象并检查
        try:
            compiler_path_obj = Path(compiler_path)
            # 检查文件是否存在（Windows下也要检查.exe版本）
            if compiler_path_obj.exists():
                return True

            # 在Windows下，如果路径不以.exe结尾，尝试添加.exe
            if platform.system().lower() == 'windows' and not compiler_path.lower().endswith('.exe'):
                exe_path = compiler_path_obj.with_suffix('.exe')
                if exe_path.exists():
                    return True

            # 即使文件不存在，如果路径结构合理，也认为是有效的
            # 这处理了路径正确但文件暂时不可访问的情况
            parent_dir = compiler_path_obj.parent
            if parent_dir.exists() and parent_dir.is_dir():
                return True

        except Exception as e:
            print(f"检查编译器路径时出错: {e}")
            return False
    ...
    def generate_clangd_config(self):
        ...
        # 添加工具链的系统头文件路径
        if compiler_path and self._is_valid_compiler_path(compiler_path):
            import os
            import glob
            ...
            # 获取编译器所在目录和工具链根目录
            compiler_dir = compiler_path_obj.parent
            toolchain_base = compiler_dir.parent

            print(f"检测到编译器路径: {compiler_path}")
            print(f"推导工具链路径: {toolchain_base}")

            # ARM工具链的标准头文件路径模式
            possible_include_patterns = [
                toolchain_base / "arm-none-eabi" / "include",
                toolchain_base / "arm-none-eabi" / "include" / "c++" / "*",
                toolchain_base / "lib" / "gcc" / "arm-none-eabi" / "*" / "include",
                toolchain_base / "lib" / "gcc" / "arm-none-eabi" / "*" / "include-fixed"
            ]

            # 搜索并添加存在的头文件路径
            for path_pattern in possible_include_patterns:
                ...
                if '*' in path_str:
                    ...
                    try:
                        ...
                        for expanded_path in sorted(expanded_paths, reverse=True):  # 优先使用较新版本
                            expanded_path_obj = Path(expanded_path)
                            if expanded_path_obj.exists() and expanded_path_obj.is_dir():
                                compile_flags.append(f"-I{expanded_path_obj}")
                                print(f"添加系统头文件路径: {expanded_path_obj}")
                                break  # 只取第一个匹配的路径（通常是最新版本）
                    except Exception as e:
                        print(f"展开路径模式 {path_str} 失败: {e}")
                        continue
                else:
                    # 对于固定路径，直接检查是否存在
                    if path_pattern.exists() and path_pattern.is_dir():
                        compile_flags.append(f"-I{path_pattern}")
                        print(f"添加系统头文件路径: {path_pattern}")
        else:
            print(f"警告: 编译器路径无效或不存在: {compiler_path}")
        ...
        if compiler_path and self._is_valid_compiler_path(compiler_path):
            ...
            for path_pattern in possible_include_patterns:
                ...
                if '*' in path_str:
                    try:
                        expanded_paths = glob.glob(path_str)
                        for expanded_path in sorted(expanded_paths, reverse=True):
                            expanded_path_obj = Path(expanded_path)
                            if expanded_path_obj.exists() and expanded_path_obj.is_dir():
                                compile_flags.append(f"-I{expanded_path_obj}")
                                break
